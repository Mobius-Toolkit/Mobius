...
- A close of the Workstream issue ends all its work. Mobius stops the Lead and all Workers, closes each open agent pull request, closes each open issue below the Workstream as "not planned", and removes all Mobius labels. Branches stay. A reopen starts a Lead again but does not reopen the closed issues.
